...
function resolveLessonIdentifier(lesson: LessonDoc): string {
  return String((lesson as any).slug || "");
}
...
const Lesson: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (isAuthed()) {
          try {
            // current behavior: get-or-create progress record
  ...
  const steps: StepSpec[] = useMemo(() => {
    ...
    if ((lesson as any).flashcards?.length) {
      out.push({
        ...
        comp: (on: ResultCb) => (
          <FlipsC
            onResult={on}
            cards={(lesson as any).flashcards.map((f: string, i: number) => ({
              id: i,
              front: f,
            }))}
          />
    ...
    if ((lesson as any).achievement) {
      out.push({
        ...
        comp: () => (
          <RewardC
            title={(lesson as any).achievement?.title || "Reward"}
    ...
    if ((lesson as any).notes) {
      out.push({
        key: "rinfo",
        graded: false,
        comp: () => <RInfoC title="Notes" description={(lesson as any).notes} />,
      });
    }
  ...
  const onExerciseResult: ResultCb = ({ result }) => {
    ...
    // non-graded content pages can continue without affecting score
    const next = step + 1;
  ...
  return (
    <Container maxWidth="md" sx={{ py: 4 }}>
      <Paper elevation={3} sx={{ p: 3, borderRadius: 3 }}>
        ...
        <LinearProgress variant="determinate" value={progressPct} sx={{ mb: 3, height: 10, borderRadius: 999 }} />
